...
- 任务中心和模型配置调整为侧边栏独立页签，与审计工作台分开展示；任务中心新增多选、全选和批量删除，活动任务自动跳过。
- 新增普通用户任务中心，以及任务搜索、筛选、查看、重命名、重试、删除和历史恢复。
- `queued/running` 状态写入数据库，API 服务重启后会自动重新调度未完成任务。
- 新增 OpenAI、DeepSeek、OpenAI-Compatible、Azure OpenAI、Ollama Provider Adapter。
- 新增增量审计基线、变更文件计算、结果差异接口及 Source-to-Sink 调用链候选节点。
- 新增上传资源与解压资源限制，以及每用户存储配额。
- 新增 LLM Token 用量与月度配额，管理员可调整用户配额。
- 模型配置页新增 All/30d/7d 用量分析、连续活跃统计、Token 热力图和模型排行。
- 新增登录会话列表、会话撤销、管理员强制下线和管理员操作日志。
- 接入 Alembic，首个 revision 为 `20260723_01`。
- 本轮按需求跳过“审计结果处置流程”。
